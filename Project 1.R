...
cereal_data1<-cereal_data[  , -c(1:3)]
head(cereal_data1)
...
ggplot(data= cereal_data1, aes(y=rating, x=fat_log)) + geom_point()+geom_smooth(method=lm)
...
set.seed(10)
test <- sample(x=nrow(cereal_data1), size = 0.20*nrow(cereal_data1))
testing <- cereal_data1[test, ]
training <- cereal_data1[-test, ]
...
plot(fit_training$residuals, pch = 16, col = "Blue")
